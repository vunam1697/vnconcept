<form action="{{ Request::fullUrl() }}" method="GET" class="form-horizontal" id="form-limit">
    ...
    <div class="rounded border border-[#D2D2D2] flex items-center select__pagination">
        <select class="select-admin" name="limit" onchange="$(this).closest('form').submit();">
            @foreach (DEFAULT_RANGE() as $v)
            <option {{ collect(Request::get('limit'))->contains($v) ? 'selected' : null }} value="{{ $v }}">{{ $v }}</option>
            @endforeach
        </select>
