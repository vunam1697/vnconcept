               	...
               	<form action="{{ route('pages.build.post') }}" method="POST">
					...
					<div class="nav-tabs-custom">
				        <ul class="nav nav-tabs">
							<li class="active">
				            	<a href="#content-1" data-toggle="tab" aria-expanded="true">Thông tin liên hệ</a>
				            </li>
				            <li class="">
                                ...
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        ...
                                        <div class="repeater" id="repeater">
                                            <table class="table table-bordered table-hover branch">
                                                <thead>
                                                    <tr>
                                                        <th width="30%">Chi nhánh</th>
                                                        <th>Cơ sở</th>
                                                        <th></th>
                                                    </tr>
                                                </thead>
                                                <tbody id="sortable">
                                                    @if (!empty($content->branch))
                                                        @foreach ($content->branch as $key => $value)
                                                            <?php $index = $loop->index + 1 ; ?>
                                                            @include('backend.repeater.row-branch')
                                                        @endforeach
                                                    @endif
                                                </tbody>
                                            </table>
                                            <div class="text-right">
                                                <button class="btn btn-primary"
                                                    onclick="repeater(event,this,'{{ route('get.layout') }}','.index', 'branch', '.branch')">Thêm
                                                </button>
                                            </div>
                                        </div>
                                    </div>
